...
- The help text of the "HTML-Bereinigung" setting is shorter and plainer.
...
### Removed
- The sanitiser allowlist "Zusätzlich erlauben" (`sanitize_allow_extra`, since 1.0.0-beta3). It was hard to use correctly; markup that must keep its `onclick` or `<iframe>` belongs in a region marked `notranslate` or `data-vtrans-exclude`, which never reaches the sanitiser. Sanitisation itself can still be switched off per connection. The column is dropped on update; a stored value is lost and its elements and attributes are removed from new translations again. This also supersedes the form tweak from [#14](https://github.com/FriendsOfREDAXO/vtrans/pull/14) by [@TobiasKrais](https://github.com/TobiasKrais), which hid the field while sanitisation was off.
